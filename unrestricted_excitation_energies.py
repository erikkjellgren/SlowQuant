...
def get_unrestricted_excitation_energy(geometry, basis, active_space, charge=0, spin=0, unit="bohr"):
    ...
    print(
        f"geometry: {geometry}, basis: {basis}, active space: {active_space}, charge: {charge}, spin (2S+1): {spin + 1}"
    )
...
def oh_radical():
    ...
    get_unrestricted_excitation_energy(
...
def h2_res():
    ...
    get_restricted_excitation_energy(
        geometry=geometry, basis=basis, active_space=active_space, charge=charge, spin=spin, unit="angstrom"
    )
