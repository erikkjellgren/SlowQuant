...
class properties():
    def __init__(
        ...
        if "do_gradients" not in self.property_options.keys():
            self.property_options["do_gradients"] = True
        # currently no option to change the rdm argument, cause i have to set it up to give errros for LR formulations that don't have RDMS
    ...
    def LR_singlet(self) -> np.ndarray:
        ...
        if self._LR_singlet is None:
            self._LR_singlet = self.property_options["lr_formulation"].quantumLR(
                self.wf, 
                excitations=self.property_options["excitations"], 
                triplet=False
            )
            self._LR_singlet.run(
                do_gradients = self.property_options["do_gradients"],
            )
    ...
    def LR_triplet(self) -> np.ndarray:
        ...
        if self._LR_triplet is None:
            self._LR_triplet = self.property_options["lr_formulation"].quantumLR(
                self.wf, 
                excitations=self.property_options["excitations"], 
                triplet=True
                )
