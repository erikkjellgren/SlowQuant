...
from slowquant.qiskit_interface.linear_response.generalized_lr_baseclass import (
...
from slowquant.unitary_coupled_cluster.generalized_density_matrix import (
    get_orbital_gradient_response_real_imag,
    get_orbital_response_hessian_block,
    get_orbital_response_metric_sigma,
    get_orbital_response_property_gradient_annika,
)
...
class quantumLR(quantumLRBaseClass):
    def run(
        ...
        finite_excitations = []
        if len(self.q_ops) != 0:
            A_matrix = get_orbital_response_hessian_block(
                self.wf.h_mo,
                self.wf.g_mo,
        ...
        for i, q in enumerate(self.q_ops):
            if abs(A_matrix[i, i]) > 10**-6:  # whatever rimeligt threshold
        ...
        for i, G in enumerate(self.G_ops):
            # <0| Gd H G |0>
            A = self.wf.QI.quantum_expectation_value_complex(
                (G.dagger * self.H_1i_1a * G).get_folded_operator(*self.orbs)
            )
        ...
        if self.num_q != 0:
            if do_rdm:
                ...
                for i, op in enumerate(self.q_ops):
                    ...
                    grad[i + self.num_q] = self.wf.QI.quantum_expectation_value_complex(
        ...
        if do_gradients:
            ...
            for i, op in enumerate(self.G_ops):
                ...
                grad[i + self.num_G] = self.wf.QI.quantum_expectation_value_complex(
        ...
        if self.num_q != 0:
            if do_rdm:
                ...
                print(f"Hermiticity check of A: max|A - A†| efter qq= "
                ...
                self.H_2i_2a = generalized_hamiltonian_2i_2a( #AE
                    self.wf.h_mo,
                    self.wf.g_mo,
            ...
            for j, qJ in enumerate(self.q_ops):
                for i, GI in enumerate(self.G_ops):
                    # Make A
                    val = (
                        self.wf.QI.quantum_expectation_value_complex( #AE complex ALLE STEDER!??
       ...
       # GG
        ...
        print(f"Hermiticity check of A: max|Sigma - Sigma†| = "
            ...
            for j, qJ in enumerate(self.q_ops):
                for i, qI in enumerate(self.q_ops[j:], j):
                    ...
                    B[i][j] = B[j][i] = self.wf.QI.op_to_qbit(
                        (qI.dagger * qJ.dagger * self.generalized_hamiltonian_full_space).get_folded_operator(*self.orbs)
                    ).paulis.to_labels()
                    # Make Sigma
            ...
            for i, qI in enumerate(self.q_ops[j:], j):
                ...
                A[i, j] = A[j, i] = np.sqrt(
                    self.wf.QI.quantum_variance(
                        (qI.dagger * self.generalized_hamiltonian_full_space * qJ).get_folded_operator(*self.orbs), no_coeffs=no_coeffs
                    )
                    + self.wf.QI.quantum_variance(
                        (qI.dagger * qJ * self.generalized_hamiltonian_full_space).get_folded_operator(*self.orbs), no_coeffs=no_coeffs
                    )
                )
                # Make B
                B[i, j] = B[j, i] = np.sqrt(
    ...
    def get_transition_dipole(self, dipole_integrals: Sequence[np.ndarray]) -> np.ndarray:
        ...
        mux_op = generalized_one_elec_op_0i_0a(mux, self.wf.num_inactive_spin_orbs, self.wf.num_active_spin_orbs)
        muy_op = generalized_one_elec_op_0i_0a(muy, self.wf.num_inactive_spin_orbs, self.wf.num_active_spin_orbs)
        muz_op = generalized_one_elec_op_0i_0a(muz, self.wf.num_inactive_spin_orbs, self.wf.num_active_spin_orbs)
        ...
        for state_number in range(number_excitations):
            ...
            if self.num_G != 0:
                transition_dipole_x = self.wf.QI.quantum_expectation_value_complex(
                    commutator(mux_op, transfer_op).get_folded_operator(*self.orbs)
                )
                transition_dipole_y = self.wf.QI.quantum_expectation_value_complex(
