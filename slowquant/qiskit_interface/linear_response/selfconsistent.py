...
class quantumLR(quantumLRBaseClass):
    def run(
        ...
        for i in range(2 * self.wf.num_active_orbs):
            # REMEMBER
            # should be changed to active elec alpha, beta in the wave function
            # REMEMBER
            if i % 2 == 0 and i // 2 < self.wf.num_elec // 2:
                hf_det += "1"
                continue
            if i % 2 == 1 and i // 2 < self.wf.num_elec // 2:
                hf_det += "1"
                continue
            hf_det += "0"
        ...
        for i, G in enumerate(self.G_ops):
            ...
            coeffs, dets = get_determinant_expansion_from_operator_on_HF(
                G, self.wf.num_active_orbs, self.wf.num_active_elec // 2, self.wf.num_active_elec // 2
            )
        ...
        if do_gradients:
            ...
            for i in range(self.num_G):
                ...
                grad[i] = self.wf.QI.quantum_expectation_value_csfs(
                    self.states["HF"], self.H_0i_0a, self.states[f"G{i}"]
                )
                # <CSF| Gd Ud H U |HF>
                grad[i + self.num_G] = self.wf.QI.quantum_expectation_value_csfs(
                    self.states[f"G{i}"], self.H_0i_0a, self.states["HF"]
                )
            if len(grad) != 0:
        ...
        for j in range(self.num_G):
            for i in range(j, self.num_G):
                ...
                val = self.wf.QI.quantum_expectation_value_csfs(
                    self.states[f"G{i}"], self.H_0i_0a, self.states[f"G{j}"]
                )
                # - <CSF| GId GJ Ud H U |CSF>
                if self.states[f"Gd{j}G{i}"] != ([], []):
                    val -= self.wf.QI.quantum_expectation_value_csfs(
                        self.states[f"Gd{j}G{i}"], self.H_0i_0a, self.states["HF"]
                    )
                ...
                if self.states[f"G{j}G{i}"] != ([], []):
                    val -= self.wf.QI.quantum_expectation_value_csfs(
                        self.states[f"G{j}G{i}"], self.H_0i_0a, self.states["HF"]
                    )
