...
def build_fock_matrix(
    ...
    Q_vn = Q_vP @ c_mo
    # inactive-inactive
    F[inact, inact] = 2 * fock_active[inact, inact]
    if not do_resp:
        F[inact, inact] += 2 * fock_inactive[inact, inact]
    # inactive-active
    F[inact, act] = 2 * fock_active[inact, act]
    if not do_resp:
        F[inact, act] += 2 * fock_inactive[inact, act]
    # inactive-virtual
    F[inact, virt] = 2 * fock_active[inact, virt]
    if not do_resp:
        F[inact, virt] += 2 * fock_inactive[inact, virt]
    # active-inactive
...
def get_orbital_gradient(kappa_idx: np.ndarray, fock_mat: np.ndarray) -> np.ndarray:
