...
from slowquant.unitary_coupled_cluster.base import Hamiltonian, a_op_spin, commutator, Epq, FermionicOperator, expectation_value
...
class LinearResponseUCC:
    def __init__(self, wave_function: WaveFunctionUCC, is_spin_conserving: bool = False) -> None:
        ...
        for (_, a,i) in iterate_T1(self.wf.active_occ, self.wf.active_unocc, is_spin_conserving=is_spin_conserving):
            self.G_ops.append(FermionicOperator(a_op_spin(a, True, num_spin_orbs, num_elec))*FermionicOperator(a_op_spin(i, False, num_spin_orbs, num_elec)))
        for (_, a,i,b,j) in iterate_T2(self.wf.active_occ, self.wf.active_unocc, is_spin_conserving=is_spin_conserving):
            tmp = FermionicOperator(a_op_spin(a, True, num_spin_orbs, num_elec))*FermionicOperator(a_op_spin(b, True, num_spin_orbs, num_elec))
    ...
    def get_excited_state_overlap(self, state_number: int) -> float:
        ...
        for i, G in enumerate(self.G_ops):
            if i == 0:
                transfer_op = self.response_vectors[i,state_number]*G + self.response_vectors[i+number_excitations,state_number]*G.dagger
            else:
                transfer_op += self.response_vectors[i,state_number]*G + self.response_vectors[i+number_excitations,state_number]*G.dagger
