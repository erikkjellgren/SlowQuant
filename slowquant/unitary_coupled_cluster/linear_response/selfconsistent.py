...
class LinearResponseUCC:
    def __init__(
        ...
        U = OperatorHybrid({inactive_str+virtual_str: OperatorHybridData(inactive_str, U_matrix, virtual_str)})
        if 's' in excitations:
        ...
        for i, op in enumerate(self.q_ops):
            grad[i] = -expectation_value_hybrid_flow(self.wf.state_vector, [H_1i_1a, op.operator], self.wf.state_vector)
            grad[i + len(self.q_ops)] = expectation_value_hybrid_flow(self.wf.state_vector, [op.operator.dagger, H_1i_1a], self.wf.state_vector)
        if len(grad) != 0:
            print('idx, max(abs(grad orb)):', np.argmax(np.abs(grad)), np.max(np.abs(grad)))
        grad = np.zeros(2*len(self.G_ops))
        for i, op in enumerate(self.G_ops):
            grad[i] = -expectation_value_hybrid_flow(self.wf.state_vector, [H_1i_1a, U, op.operator], csf)
            grad[i + len(self.G_ops)] = expectation_value_hybrid_flow(csf, [op.operator.dagger, U.dagger, H_1i_1a], self.wf.state_vector)
        if len(grad) != 0:
        ...
        for j, opJ in enumerate(self.q_ops):
            ...
            for i, opI in enumerate(self.q_ops):
                ...
                if i < j:
                    continue
                # Make M
                val = expectation_value_hybrid_flow(self.wf.state_vector, [qI.dagger, H_2i_2a, qJ], self.wf.state_vector) - expectation_value_hybrid_flow(self.wf.state_vector, [qI.dagger, qJ, H_2i_2a], self.wf.state_vector) 
                self.M[i, j] = self.M[j, i] = val
                # Make Q
                self.Q[i, j] = self.Q[j, i] = -expectation_value_hybrid_flow(self.wf.state_vector, [qI.dagger, qJ.dagger, H_2i_2a], self.wf.state_vector)
                # Make V
                self.V[i, j] = self.V[j, i] = expectation_value_hybrid_flow(self.wf.state_vector, [qI.dagger, qJ], self.wf.state_vector)
        for j, opJ in enumerate(self.q_ops):
            qJ = opJ.operator
            for i, opI in enumerate(self.G_ops):
                GI = opI.operator
                # Make M
                val = expectation_value_hybrid_flow(csf, [GI.dagger, U.dagger, H_1i_1a, qJ], self.wf.state_vector) - expectation_value_hybrid_flow(csf, [GI.dagger, U.dagger, qJ, H_1i_1a], self.wf.state_vector)
                self.M[i + idx_shift, j] = self.M[j, i + idx_shift] = val
                # Make Q
                self.Q[i + idx_shift, j] = self.Q[j, i + idx_shift] = -expectation_value_hybrid_flow(csf, [GI.dagger, U.dagger, qJ.dagger, H_1i_1a], self.wf.state_vector)
        for j, opJ in enumerate(self.G_ops):
            ...
            for i, opI in enumerate(self.G_ops):
                ...
                if i < j:
                    continue
                # Make M
                val = expectation_value_hybrid_flow(csf, [GI.dagger, U.dagger, H_en, U, GJ], csf) - expectation_value_hybrid_flow(csf, [GI.dagger, GJ, U.dagger, H_en], self.wf.state_vector)
                ...
                self.Q[i + idx_shift, j + idx_shift] = self.Q[
                    j + idx_shift, i + idx_shift
                ] = -expectation_value_hybrid_flow(csf, [GI.dagger, GJ.dagger, U.dagger, H_en], self.wf.state_vector)
                # Make V
                if i == j:
