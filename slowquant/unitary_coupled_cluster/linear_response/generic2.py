    ...
    def __init__(
        ...
        if use_matrix_symmetry:
            ...
            for j, GJ in enumerate(self.G_ops):
                for i, qI in enumerate(self.q_ops):
                    # Make A
                    self.A[i, j + idx_shift] = self.A[j + idx_shift, i] = expectation_value_hybrid_flow_double_commutator(
                        self.wf.state_vector,
                        qI.dagger, H_1i_1a, GJ,
                        self.wf.state_vector,
                    )
                    # Make B
                    self.B[i, j + idx_shift] = self.B[j + idx_shift, i] = expectation_value_hybrid_flow_double_commutator(
                        self.wf.state_vector,
