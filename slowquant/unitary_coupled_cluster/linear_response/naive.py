...
from slowquant.unitary_coupled_cluster.operators import Epq, Tpq, one_elec_op_0i_0a
    ...
    def get_transition_dipole(self) -> np.ndarray:
        ...
        for state_number in range(number_excitations):
            ...
            for i, G in enumerate(self.G_ops):
                transfer_op += self.Z_G_normed[i, state_number] * G.dagger
                if not self.tda:
                    transfer_op += self.Y_G_normed[i, state_number] * G
            q_part_x = 0.0
            q_part_y = 0.0
            q_part_z = 0.0
            if len(self.q_ops) != 0:
        ...
        for idx, G in enumerate(self.G_ops):
            ...
            for p in range(self.wf.num_inactive_orbs, self.wf.num_inactive_orbs + self.wf.num_active_orbs):
                for q in range(
                    ...
                    E_ket = propagate_state([pq(p, q)], self.wf.ci_coeffs, *self.index_info)
