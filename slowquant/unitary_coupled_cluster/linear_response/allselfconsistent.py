...
class LinearResponse(LinearResponseBaseClass):
    ...
    def get_property_gradient(self, int1e: np.ndarray, int2e: np.ndarray | None = None) -> np.ndarray:
        ...
        for comp, op_int1e in enumerate(int1e):
            ...
            if int2e is None:
                op = one_elec_op_1i_1a(op_int1e, self.wf.num_inactive_orbs, self.wf.num_active_orbs, triplet=self.triplet)
            else:
                op = hamiltonian_1i_1a(op_int1e, int2e[comp], self.wf.num_inactive_orbs, self.wf.num_active_orbs)
            Udop_ket = propagate_state(["Ud", op], self.wf.ci_coeffs, *self.index_info)
            ...
            for idx, q in enumerate(self.q_ops):
                ...
                qd_ket = propagate_state([q.dagger], self.wf.csf_coeffs, *self.index_info)
                # < CSF | q Ud op | 0 >
            ...
            if int2e is None:
                op = one_elec_op_0i_0a(op_int1e, self.wf.num_inactive_orbs, self.wf.num_active_orbs, triplet=self.triplet)
            else:
            ...
            for idx, G in enumerate(self.G_ops):
                ...
                Gd_ket = propagate_state([G.dagger], self.wf.csf_coeffs, *self.index_info)
                # < CSF | G Ud op | 0 >
