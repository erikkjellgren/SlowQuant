...
class LinearResponseBaseClass:
    ...
    def __init__(
        ...
        do_tda: bool = False,
        do_spinadapted_singlet_orbital_response: bool = True,
    ) -> None:
        ...
        self._s2_is_identity = False
        self.do_tda = do_tda
        self.do_orbital_response = do_orbital_response
        ...
        if self.do_orbital_response:
            for p, q in self.wf.kappa_no_activeactive_idx:
                if do_spinadapted_singlet_orbital_response:
                    self.q_ops.append(G1_sa(p, q))
                else:
                    self.q_ops.append(G1(p//2, q//2))
                    self.q_ops.append(G1(p//2+1, q//2+1))
    ...
    def solve_linear_response(self) -> None:
        """Calculate excitation energies."""
        if self.do_tda:
            # This code path will also be used by parameterizations where B=0.
            self.E2 = self.A
            ...
            if not self._s2_is_identity:
                eigval, eigvec = scipy.linalg.eig(self.E2, self.S2)
            else:
                eigval, eigvec = np.linalg.eigh(self.E2)
