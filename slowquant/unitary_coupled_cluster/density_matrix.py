...
def get_orbital_gradient_response(
    ...
    """
    gradient = np.zeros(2 * len(kappa_idx), dtype=np.float64)
    for idx, (m, n) in enumerate(kappa_idx):
        # 1e contribution
        for p in range(num_inactive_orbs + num_active_orbs):
...
def get_orbital_response_metric_sigma(
    ...
    """
    sigma = np.zeros((len(kappa_idx), len(kappa_idx)), dtype=np.float64)
...
def get_orbital_response_hessian_block(
    ...
    """
    A1e = np.zeros((len(kappa_idx1), len(kappa_idx1)), dtype=np.float64)
    A2e = np.zeros((len(kappa_idx1), len(kappa_idx1)), dtype=np.float64)
