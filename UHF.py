...
def HartreeFock(input, set, basis, VNN, Te, S, VeN, Vee, results, print_SCF='Yes'):
    ...
    for iter in range(1, Maxiter):
        ...
        #New Fock Matrix
        Part_alpha = np.zeros((len(basis),len(basis)))
        Part_alpha_beta = np.zeros((len(basis),len(basis)))
        Part_beta = np.zeros((len(basis),len(basis)))
        Part_beta_alpha = np.zeros((len(basis),len(basis)))
        ...
        rmsD_alpha = math.sqrt(rmsD_alpha)
        rmsD_beta = 0
        for i in range(0, len(D0_beta)):
            for j in range(0, len(D0_beta[0])):
                rmsD_beta += (D_beta[i,j] - D0_beta[i,j])**2
        rmsD_beta = math.sqrt(rmsD_beta)
        ...
        D0_beta = D_beta
        E0el = Eel
        if dE < 10**(-deTHR) and rmsD_alpha < 10**(-rmsTHR) and rmsD_beta < 10**(-rmsTHR):
            break
            
    if print_SCF == 'Yes':
        output.write('\n \n')
        output.close()
    results['UHFenergy'] = Eel+VNN[0]
    
    return C_alpha, F_alpha, D_alpha, C_beta, F_beta, D_beta, results
