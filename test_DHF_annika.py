...
get_gradient_finite_diff, get_electronic_energy_generalized
)
...
def NR(geometry, basis, active_space, unit="bohr", charge=0, spin=0, c=137.036):
    ...
    h_core = mf.get_hcore()
    g_eri = np.array([mol.intor("int2e_spinor"), mol.intor('int2e_spsp1spsp2_spinor')*(0.125/c**4),
                      mol.intor('int2e_spsp2_spinor')*(0.25/c**2), mol.intor('int2e_spsp1_spinor')*(0.25/c**2)])
    ...
    H = generalized_hamiltonian_full_space(h_mo[size:,size:], g_mo[size:,size:,size:,size:], WF.num_spin_orbs_PES)
    ...
    WF.run_wf_optimization_1step("l-bfgs-b", orbital_optimization=True, tol=1e-10, maxiter = 500)
    #WF.do_adapt(["S","D"])
