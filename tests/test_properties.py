...
def test_properties_ch3_sto3g() -> None:
    ...
    Does NOT give the same as in the reference. (Reference is: 0.0152)
    ...
    A.set_molecule(
        """C   0.000000000000  0.000000000000   0.000000000000;
           ...
           H  -1.095000000000  1.765825798316   0.000000000000;
           H  -1.095000000000 -1.765825798316   0.000000000000""",
        distance_unit="bohr",
    )
    A.set_basis_set("sto-3g")
...
def test_properties_ch3_431g() -> None:
    ...
    Does NOT give the same as in the reference. (Reference is: 0.0122)
    Reference data: Szabo and Ostlund
    """
    A = sq.SlowQuant()
    A.set_molecule(
    ...
    A.hartree_fock.run_unrestricted_hartree_fock()
    A.init_properties()
    assert abs(A.hartree_fock.spin_contamination - 0.013124715896593031) < 10**-4


def test_properties_ch3_631gs() -> None:
    """Test unrestricted Hartree-Fock for CH3 with 6-31G*.

    Does NOT give the same as in the reference. (Reference is: 0.0118)
    Reference data: Szabo and Ostlund
    """
    A = sq.SlowQuant()
    A.set_molecule(
    ...
    A.hartree_fock.run_unrestricted_hartree_fock()
    A.init_properties()
    assert abs(A.hartree_fock.spin_contamination - 0.012599564332392621) < 10**-4


def test_properties_ch3_631gss() -> None:
    """Test unrestricted Hartree-Fock for CH3 with 6-31G**.

    Does NOT give the same as in the reference. (Reference is: 0.0114)
    Reference data: Szabo and Ostlund
    """
    A = sq.SlowQuant()
    A.set_molecule(
    ...
    A.hartree_fock.run_unrestricted_hartree_fock()
    A.init_properties()
    assert abs(A.hartree_fock.spin_contamination - 0.01215019463895084) < 10**-4
